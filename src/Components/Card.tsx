...
interface CardProps {
  ...
  title: string;
  description: string;
  new_price: string;  // Change to number here
...
function Card({
  ...
  const handleAddToCart = () => {
    addToCart({
      ...
      name: title,  // or any other field you want to use as the name
    });
  };
  ...
  return (
    <div className="relative group border bg-[#F4F5F7] h-[450px] w-[280px] shadow-md">
      ...
      <div className="relative h-[300px] overflow-hidden">
        <Image src={image} alt={label} className="h-full w-full object-cover" />
      ...
      <div className="pl-5 pt-4 pb-4">
        ...
        <p className="text-[#898989] text-[16px] font-medium">{description}</p>

        {/* Price */}
        <div className="mt-2 flex items-center space-x-2">
          <span className="font-semibold text-[20px]">
            Rp {new_price.toLocaleString()} {/* Use new_price directly */}
          </span>
          {old_price && (
            <span className="text-gray-400 line-through text-[16px]">
              Rp {old_price.toLocaleString()} {/* Use old_price directly */}
            </span>
          )}
        </div>
      </div>
